...
> **Nota**: En este ejercicio se presupone una familiarización con Fundición de IA de Azure, por lo que algunas instrucciones son menos detalladas, de forma intencionada, para fomentar una exploración más activa y el aprendizaje práctico.

## Introducción

En este ejercicio, habilitarás la supervisión de una aplicación de finalización de chat y verás su rendimiento en Azure Monitor. Interactúas con el modelo implementado para generar datos, ver los datos generados a través del panel Insights para aplicaciones de IA generativa y configurar alertas para ayudar a optimizar la implementación del modelo.
...
Para ver los datos recopilados de las interacciones del modelo, tendrá acceso al panel que se vincula a un libro de Azure Monitor.
...
> Úsalo para identificar solicitudes problemáticas o errores de configuración. Es probable que se produjera un error en la última solicitud porque era demasiado larga.
...
> Úsala para evaluar el nivel de detalle, la relevancia y la coherencia. Especialmente en relación con los recuentos de tokens y la duración.

## 6. (OPCIONAL) Creación de una alerta

Si te sobra tiempo, intenta configurar una alerta para que recibas una notificación cuando la latencia del modelo supere un umbral determinado. Se trata de un ejercicio diseñado para desafiarte, lo que significa que las instrucciones son intencionadamente menos detalladas.
